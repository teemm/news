...
/*	$percentChange = $scaledSize['w'] / 500; // we know we scaled by width of 500 in upload
	...
	*/
		$percentChange = $scaledSize['w'] ; // we know we scaled by width of 500 in upload
	$newCoords = array(
		'x' => $v['x'] ,
		'y' => $v['y'],
		'w' => $v['w'],
	...
	// 4) resize the cropped image to whatever size we need
	$ar = $gd->getAspectRatio($newCoords['w'], $newCoords['h'], $newCoords['w'], 0);
